// ================================================================
//  SLOW LEARNER PROGRESS MONITORING MODAL EDITOR
// ================================================================
...
function getSlowLearnerEntries() {
    try {
        ...
        // If old sample data with P. Marveth Sai or Haavesh exists, clear it to start clean & empty for the teacher
...
function openSlowLearnerModal() {
    const modal = document.getElementById('slow-learner-modal');
    if (!modal) return;
    ...
    const entries = getSlowLearnerEntries();
    renderModalRows(entries);

    modal.style.display = 'flex';
}

function closeSlowLearnerModal() {
    const modal = document.getElementById('slow-learner-modal');
...
function saveAllSlowLearnerRows() {
...
function renderSlowLearnerTab() {
    const container = document.getElementById('slowLearnerTableBody');
    if (!container) return;

    const entries = getSlowLearnerEntries().filter(e => e.studentName || e.learningGap);
    const badge = document.getElementById('slowLearnerBadge');
        ...
        container.innerHTML = `
            <tr>
                <td colspan="8" style="text-align:center; padding: 32px; color: var(--text-muted);">
                    🔍 No slow learner records found. Click <strong>"🌱 Slow Learner Progress"</strong> to open the editor.
                </td>
            </tr>
        `;
        return;
    ...
    container.innerHTML = entries.map((row) => {
        let badgeClass = 'badge-improving';
        if (row.progress === 'Improving') badgeClass = 'badge-improving';
        else if (row.progress === 'Satisfactory') badgeClass = 'badge-satisfactory';
        else if (row.progress === 'Little Improvement') badgeClass = 'badge-little';
        else if (row.progress === 'Needs Attention') badgeClass = 'badge-attention';

        return `
            <tr>
                <td style="font-weight: 500; font-size: 13px;">${escapeHtml(row.date || '')}</td>
                <td style="font-weight: 600; color: var(--text-main);">${escapeHtml(row.studentName || '')}</td>
                <td><span class="sl-subject-tag">${escapeHtml(row.subject || '')}</span></td>
                <td>${escapeHtml(row.learningGap || '')}</td>
                <td>${escapeHtml(row.strategy || '')}</td>
                <td><span class="sl-progress-badge ${badgeClass}">${escapeHtml(row.progress || '--')}</span></td>
                <td>${escapeHtml(row.nextStep || '')}</td>
                <td class="sl-actions-cell">
                    <button class="btn-icon-sm" onclick="openSlowLearnerModal()" title="Edit Records">✏️</button>
                </td>
            </tr>
        `;
    }).join('');
...
function printSlowLearnerSheet() {
    window.print();
}
...
window.closeSlowLearnerModal = closeSlowLearnerModal;
...
window.renderSlowLearnerTab = renderSlowLearnerTab;
window.printSlowLearnerSheet = printSlowLearnerSheet;
